plot.survfitJM <-
function (x, estimator = c("both", "mean", "median"), which = NULL, fun = NULL, conf.int = TRUE,
    ...
    for (i in seq_along(which)) {
        ii <- which[i]
        r <- x$summaries[[ii]]
        r <- rbind(cbind(c(0, x$last.time[ii]), matrix(1, 2, 4)), r)
        ...
        if (include.y)
            layout(matrix(c(1, 2)))
        ylim <- if (is.null(fun)) c(0, 1) else { rr <- r[, -1]; range(rr[is.finite(rr)]) } 
        matplot(r[, 1], r[, -1], type = "l", col = col, lty = lty, ylim = ylim, main = main[ii], xlab = xlab[i], ylab = ylab[i], ...)
